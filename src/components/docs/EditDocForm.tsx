...
import {
  ArrowLeft, Save, X, Loader2, Camera, Upload,
  Wrench, Zap, Bug, StickyNote, FileText, MessageSquare,
} from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import type { DocEntryWithDetails } from "@/types";
...
interface Props {
  doc: DocEntryWithDetails;
}

export function EditDocForm({ doc }: Props) {
  ...
  const [images, setImages] = useState<ImagePreview[]>(
    doc.attachments?.map((att) => ({
      ...
      type: att.type as "before" | "after" | "screenshot",
  ...
  async function handleImageAdd(e: React.ChangeEvent<HTMLInputElement>, imgType: "before" | "after" | "screenshot") {
    ...
    for (const file of files) {
      // 1. Adiciona preview local imediatamente
      const previewUrl = URL.createObjectURL(file);
      const tempEntry: ImagePreview = {
        url: "",
        previewUrl,
        type: imgType,
        fileName: file.name,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      // 1. Atualiza a documentação básica
      ...
      if (!res.ok) throw new Error(error ?? "Erro ao salvar");

      // 2. Identifica e salva novos anexos (que não possuem id)
      ...
      // 3. Identifica e deleta anexos removidos
      const existingAttachmentIds = doc.attachments?.map((a) => a.id) ?? [];
      const currentAttachmentIds = images.map((img) => img.id).filter(Boolean) as string[];
      const deletedIds = existingAttachmentIds.filter((id) => !currentAttachmentIds.includes(id));

      for (const id of deletedIds) {
        await fetch(`/api/docs/attachments?id=${id}`, {
          method: "DELETE",
        });
      }
      ...
      toast.error(err.message ?? "Erro ao atualizar documentação");
    } finally {
      setSaving(false);
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full h-11 bg-card border border-border rounded-xl px-4 text-sm focus:outline-none focus:border-primary transition-colors placeholder:text-muted-foreground/60"
          />
        </div>
        ...
        <div>
          <label className="text-sm font-medium mb-1.5 block">
            Resumo <span className="text-muted-foreground font-normal">(aparece na listagem)</span>
          </label>
          <input
            value={summary}
        ...
        {/* Before & After images (for refactoring) */}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="min-h-24 border-2 border-dashed border-border rounded-xl p-2 space-y-2">
                  <label className="flex flex-col items-center justify-center h-16 w-full cursor-pointer text-muted-foreground hover:text-foreground transition-colors">
                    <Upload className="w-4 h-4 mb-1" />
                    <span className="text-xs">Adicionar Antes</span>
        ...
        {/* Screenshots (all types) */}
        <div>
          <label className="text-sm font-medium mb-2 block">
            Screenshots <span className="text-muted-foreground font-normal text-xs">(opcional)</span>
          </label>
          <div className="flex flex-wrap gap-2">
            {screenshots.map((img, idx) => (
              <ImagePreviewCard key={idx} img={img} small onRemove={() => removeImage(images.indexOf(img))} />
            ))}
            <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-border rounded-xl cursor-pointer text-xs text-muted-foreground hover:text-foreground hover:border-primary/40 transition-colors">
...
function ImagePreviewCard({
  ...
  return (
    <div className={cn("relative group", small ? "w-16 h-16" : "w-full")}>
      ...
      <img
        ...
        className={cn(
          ...
          small ? "w-16 h-16" : "w-full max-h-32 object-cover",
